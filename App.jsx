    ...
    <div style={{ height: '100vh', width: '100vw', position: 'relative', background: '#0a0a0a' }}>
      ...
      <MapContainer center={playerPos} zoom={15} style={{ height: '100%', width: '100%' }}>
        ...
        <Marker position={playerPos} icon={L.divIcon({
          html: `<div style="font-size: 32px; filter: drop-shadow(0 0 6px #00ffff)">👤<br/><span style="font-size:11px; color:#00ffff; font-family:monospace">${playerName}</span></div>`,
          iconSize: [60, 50],
          className: ''
        })}>
        ...
        <Marker key={s.player_id} position={[s.latitude, s.longitude]} icon={L.divIcon({
          html: `<div style="font-size: 40px; filter: drop-shadow(0 0 6px red)">🐌<br/><span style="font-size:11px; color:#ff4444; font-family:monospace">${s.player_id}'s SNAIL</span></div>`,
          iconSize: [80, 60],
          className: ''
        })}>
          <Popup>🐌 別人的蝸牛</Popup>
